...
- 身份认证、权限校验：网关作为微服务入口，需要校验用户是否有请求资格，如果没有则进行拦截。
...
> 将分布式事务拆分为本地事务和消息事务两部分
>
> - 本地事务在本地进行提交和回滚
> - 消息事务将消息写入消息中间件中，以实现消息的可靠投递与顺序性



**实现方案**

1、发送消息之前，创建一条本地消息，保证本地业务数据的写操作和本地消息的写操作在同一个事物中。这样就能保证只要业务操作成功，本地消息写成功

2、基于本地消息，使用MQ进行发送，消费者来消费消息，消费者拿到消息后，进行对应的业务处理，处理成功后，修改本地消息表的状态

![](https://cdn.jsdelivr.net/gh/xrj123123/Images/202409181837253.png)

**失败处理**

- 步骤1、2失败，因为在同一个事物中，事物会回滚
- 步骤3失败，通过一个定时任务，定时扫描本地消息表，将未处理的消息进行发送
- 步骤4、5失败，基于MQ的重试机制保证
- 步骤6、7失败，此时两个系统的数据已经一致，但是本地消息表的状态是错误的。此时该消息会重新发送，然后执行，下游需要保证幂等性，然后再次修改本地消息表的状态。或者本系统定时查询下游系统状态，来更新消息状态
...
> 本地消息表中的数据越来越多，定时任务扫表越来越慢
>
> 解决方案
>
> - 在`state`字段上添加索引。虽然区分度不高，但一般情况下，`success`的数据占90%，`init`数据占10%，只需要关心`init`的数据即可，所以加索引后，效率是可以提升的
> - 多线程并发扫表。分段扫描
...
>
> - 数据库做主从集群，每次扫描时，扫描从库。如果扫描从库后，还需要将数据写入，那么此时需要写入主库。这时，可以通过分库处理，将一个库的数据分散到多个库中，提高吞吐量。分为多个数据库时，数据库的连接数就会增多 

3、集中式扫表会影响正常业务

> - 定时任务都是集中式的定时执行的，那么就会存在延迟的问题。随着数据库越来越大，延时会越来越长。想要降低延迟，那就要抛弃定时任务的方案，可以考虑延迟消息，基于延迟消息来做定时执行。用了延迟消息之后，还可以缓解数据库的压力。也能比定时扫表的性能要好，实时性也更高。 
>
> - 分批次执行 
...
![](https://cdn.jsdelivr.net/gh/xrj123123/Images/202409182049386.png)

> 在最大努力通知的过程中，可能会出现消息重复发送的情况，也可能会出现消息丢失的情况。因此，在设计最大努力通知系统时，需要根据实际业务需求和风险承受能力来确定最大努力通知的策略和重试次数,以及对消息进行去重等处理。
>
...
1. `Cancel`：如果任何一个参与者在try阶段执行失败，则协调者通知所有参与者回滚事务。那么就要执行cancel
阶段。
...
如果Confirm阶段失败，通常是在提交事务时出现了问题

- 重试：适用于解决临时问题（网络延迟、服务短暂不可用）导致的失败。在重试之前，可以设定一个延迟或等待一段时间，然后再次尝试Confirm操作。通常，会设置重试次数的上限，以避免无限重试
...
- 执行Cancel操作：如果重试Confirm操作依然失败，或者系统确定Confirm无法成功，就执行Cancel操作。Cancel阶段的目的是撤销在Try阶段预留的所有资源，确保系统回到事务开始前的状态。这是一种典型的回滚操作，用于处理事务失败的情况。
- 日志记录和异常监控：在Confirm失败的情况下，重要的是记录详细的错误日志和监控异常。这可以帮助系统管理员或开发人员分析为什么Confirm操作失败，并采取相应的改进措施。此外，日志可以帮助在事后定位问题的根源。



**Cancel失败怎么办？**

- 记录日志&告警：将错误信息记录下来，方便后续分析和处理。并及时通知相关人员进行处理。
...
柔性事务是业内解决分布式事务的主要方案。柔性事务，相比较与数据库事务中的ACID这种刚性事务来说，柔性事务保证的是**基本可用，最终一致**。这其实就是基于BASE理论，保证数据的最终一致性。

柔性事务主要有以下三种类型

- 异步确保型：通过异步执行来保证事务的最终一致性。将一些同步阻塞的事务操作变为异步的操作，避免对数据库事务的占用。比如热点资源的批量更新、异步更新的处理。
- 补偿性：通过记录补偿操作来撤销之前的修改，保证事务的原子性
- 最大努力通知型：适用于一些最终一致性事件敏感度低的业务，通过最大努力将业务处理结果通知到接收方
